...
In the 'Client Function' from the example below, we get the DeviceId from the Queue message to construct the EntityId. Then the `SignalEntityAsync()` is called with 2 arguments, first the DeviceId (and the name of the entity) and the secondly the namf of the method that we want to invoke.
